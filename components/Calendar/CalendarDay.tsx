...
const CalendarDay = React.memo(
  ({
    ...
    const dynamicStyles = useMemo(() => {
      ...
      return styles;
    }, [date, isPeriod, isPrediction, isSelected, isInRange, themeColor]);

    // Handle empty days (padding at start of month) - AFTER HOOKS
    if (!date) {
    ...
    const containerBase =
      "w-[14.28%] aspect-square justify-center items-center mb-1 rounded-full";
    ...
    if (isSelectionStart && !isSelectionEnd)
      borderRadiusClass = "rounded-l-full rounded-r-none";
    // Only round right side for end day of the selection
    if (isSelectionEnd && !isSelectionStart)
      borderRadiusClass = "rounded-r-full rounded-l-none";

    // Text styling
    const textClasses = clsx("text-xl font-bold", {
    ...
    return (
      <View className={containerBase} ref={viewRef} collapsable={false}>
        <TouchableOpacity
          onPress={handlePress}
          activeOpacity={0.7}
          ...
          style={dynamicStyles}
        >
          <Text className={textClasses}>{format(date, "d")}</Text>
        </TouchableOpacity>
      </View>
    );
  },
